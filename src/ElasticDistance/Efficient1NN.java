...
//        maybe a different version which looks for missing files and runs them?
        ...
        String stepPathDir = TSCConfigures.step+totalClassName[totalClassName.length-1]+"/"+datasetName;
...
//    }
    ...
    public void UCR2018ClsSavePathAll(boolean cv, boolean resample100){
        ...
        try {
            br = new BufferedReader(new FileReader(new File(TSCConfigures.dataForJavaCls)));
            String datasetName;
    ...
    public void UCR2018Cls(boolean cv, boolean resample100,String datasetSavePath,boolean stepAnalyze){
        ...
        String distPathDir = TSCConfigures.distPath+totalClassName[totalClassName.length-1]+"/"+datasetSavePath;
        if(!datasetSavePath.equals("None")){
            resCls = new File(TSCConfigures.distPath+totalClassName[totalClassName.length-1]+"/"+datasetSavePath+"/timestamp.txt");
            if(!resCls.getParentFile().exists()){
                resCls.getParentFile().mkdirs();
            }
            resCls = new File(TSCConfigures.distPath+totalClassName[totalClassName.length-1]+"/datasetDone.txt");
            if(stepAnalyze) {
                resCls = new File(TSCConfigures.distPath + totalClassName[totalClassName.length - 1] + "/datasetDone.txt");
                if(!resCls.exists()){
                    if(!resCls.getParentFile().exists()){
...
//
            String tscProbDir = TSCConfigures.tscProbDir;
            BufferedReader br =new BufferedReader(new FileReader(new File(TSCConfigures.dataForJavaClsStep)));
            ...
            while((line=br.readLine())!=null){
                ...
                if(!datasetSavePath.equals("None")){
                    resCls = new File(TSCConfigures.distPath+totalClassName[totalClassName.length-1]+"/datasetDone.txt");
                    if(stepAnalyze)
                        resCls = new File(TSCConfigures.step+totalClassName[totalClassName.length-1]+"/datasetDone.txt");
                    bw.flush();
                    bw.close();
