...
import timeseriesweka.classifiers.ensembles.elastic_ensemble.CDTW1NN;
...
import timeseriesweka.classifiers.CDD_DTW;
...
//first gives the problem file      
        ...
        System.out.println("214\t"+predictions);
        f=new File(predictions);
        if(!f.exists())
            f.mkdir();
        predictions=predictions+"/"+problem; // problem is name of dataset
        f=new File(predictions);
        if(!f.exists())
            f.mkdir();
...
// Save internal info for ensembles
        System.out.println("263\t"+(c instanceof SaveableEnsemble));
        ...
        try{              
            ...
            System.out.println("292\t"+data[1].numInstances());
...
//1. Set up the paths
        ...
        DataSets.resultsPath="D:/datasets/Univariate_arff_results/";
//2. Set up the arguments: Classifier, Problem, Fold
        //String[] paras={"BOSS","ItalyPowerDemand","1"};
        String name = "MiddlePhalanxTW";
        String[] paras={"DD_DTW",name,"1"};
//3. Run a full experiment, saving the results
        singleClassifierAndFold(paras);
        String[] paras2={"CDD_DTW",name,"1"};
//3. Run a full experiment, saving the results
        singleClassifierAndFold(paras2);
    }
