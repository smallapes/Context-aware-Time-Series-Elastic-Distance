 ...
 * L =[1,2,3,4,5]
 */
public class CDTWCV1NNConv2Drop extends Efficient1NN {
    private double r;
    private double step;
    private int L;
     ...
     * @param r
     */
    public CDTWCV1NNConv2Drop(double r){
    ...
    //for varying window sizes
        ...
        for(int i=0,j=0;i<1;i++){
            ...
            for(int l=1;l<=this.L;l++){
                if (i-step*l>=0&&j-step*l>=0){
                    matrixD[i][j] += Math.pow((first.value(Math.max(0,i-step*l)) - second.value(Math.max(0,j-step*l))),2);
                    count +=1;
                }
                if (i+step*l<n||j+step*l<m){
                    matrixD[i][j] += Math.pow((first.value(Math.min(n-1,i+step*l)) - second.value(Math.min(m-1,j+step*l))),2);
                    count +=1;
                }
            }
            matrixD[i][j] /= count;
...
//        String datasetName = "SonyAiboRobotSurface1";
        ...
        // new version
        CDTWCV1NNConv2Drop dtwNew = new CDTWCV1NNConv2Drop(r);
    ...
    public static void UCR2018(){
        ...
        Efficient1NN dtwNew = new CDTWCV1NNConv2Drop();
...
//            UCR2018StepAnalyze();
    ...
    public void setParamsFromParamId(Instances train, int paramId) {
        if(this.allowLoocv){
            ...
            this.L = (int)Math.floor(paramId/10)+1;
