 ...
 * @author sjx07ngu
 */
public class CTWE1NN extends Efficient1NN{
    ...
    public CTWE1NN(double nu, double lambda){
        this.nu = nu;
        this.lambda = lambda;
...
//CHANGE AJB 8/1/16: Only use power of 2 for speed
                    ...
                    distj1 += ((tb[j - 2][k] - tb[j - 1][k]) * (tb[j - 2][k] - tb[j - 1][k])+
                                Math.pow(tb[Math.max(j - 2-step,0)][k] - tb[Math.max(j - 1-step,0)][k],2)+
                                Math.pow(tb[Math.min(j - 2+step,c-1)][k] - tb[Math.min(j - 1+step,c-1)][k],2))/3;
...
// in c:               distj1+=pow(fabs(tb[j-2][k]-tb[j-1][k]),degree);
                    ...
                    distj1 += (tb[j - 1][k] * tb[j - 1][k]+
                            Math.pow(tb[Math.max(j - 1-step,0)][k],2)+
                            Math.pow(tb[Math.max(j - 1+step,c-1)][k],2))/3;
                }
            }
            ...
            for (k = 0; k < dim; k++) {
                if (i > 1) {
                    ...
                    disti1 += ((ta[i - 2][k] - ta[i - 1][k]) * (ta[i - 2][k] - ta[i - 1][k])+
                            Math.pow(ta[Math.max(i - 2-step,0)][k] - ta[Math.max(i - 1-step,0)][k],2)+
                            Math.pow(ta[Math.min(j - 2+step,r-1)][k] - ta[Math.min(j - 1+step,r-1)][k],2))/3;
                } // OLD                 disti1+=Math.pow(Math.abs(ta[i-2][k]-ta[i-1][k]),degree);
                else {
                    //tcl
                    disti1 += ((ta[i - 1][k]) * (ta[i - 1][k])+
                            Math.pow(ta[Math.max(i - 1-step,0)][k],2)+
                            Math.pow(ta[Math.max(i - 1+step,r-1)][k],2))/3;
                }
            }
            ...
            for (j = 1; j <= c; j++) {
                ...
                for (k = 0; k < dim; k++) {
                    ...
                    dist += ((ta[i - 1][k] - tb[j - 1][k]) * (ta[i - 1][k] - tb[j - 1][k])+
                            Math.pow(ta[Math.max(i - 1-step,0)][k] - tb[Math.max(j - 1-step,0)][k],2)+
                            Math.pow(ta[Math.min(i - 1+step,r-1)][k] - tb[Math.min(j - 1+step,c-1)][k],2))/3;
//                  dist+=Math.pow(Math.abs(ta[i-1][k]-tb[j-1][k]),degree);
                    if (i > 1 && j > 1) {
                        dist += ((ta[i - 2][k] - tb[j - 2][k]) * (ta[i - 2][k] - tb[j - 2][k])+
                                Math.pow(ta[Math.max(i - 2-step,0)][k] - tb[Math.max(j - 2-step,0)][k],2)+
                                Math.pow(ta[Math.min(i - 2+step,r-1)][k] - tb[Math.min(j - 2+step,c-1)][k],2))/3;
                    }
...
//        String datasetName = "SonyAiboRobotSurface1";
        ...
        // new version
        CTWE1NN dtwNew = new CTWE1NN(0.001,0.5);
