...
/**
 * @author tcl
 * validate for the slope weight to context point of current point
 * abandoned
 * SlopeWeight = [0,....,1.5*0.9]
 * abandoned
 */
public class CDTWSlopeCV1NN extends Efficient1NN {
    private double r;
    private double step;
     ...
     * @param r
     */
    public CDTWSlopeCV1NN(double r){
...
//    }
    ...
    //for varying window sizes
        ...
        for(int i=0,j=0;i<1;i++){
            matrixD[0][0] = (Math.pow((first.value(i) - second.value(j)),2)*SlopeWeight+
                    Math.pow((first.value(Math.max(0,i-step)) - second.value(Math.max(0,j-step))),2)*(3-2*SlopeWeight)+
                    Math.pow((first.value(Math.min(n-1,i+step)) - second.value(Math.min(m-1,j+step))),2)*SlopeWeight)/3;
        }
    ...
    //Warp first.value(0] onto all second.value(1]...second.value(r+1]
        for (int j = 1; j < windowSize && j < m; j++) {
            int i = 0;
            matrixD[0][j] = matrixD[0][j - 1] + (Math.pow((first.value(i) - second.value(j)),2)*SlopeWeight+
                    Math.pow((first.value(Math.max(0,i-step)) - second.value(Math.max(0,j-step))),2)*(3-2*SlopeWeight)+
                    Math.pow((first.value(Math.min(n-1,i+step)) - second.value(Math.min(m-1,j+step))),2)*SlopeWeight)/3;
        }
    ...
    //	Warp second.value(0] onto all first.value(1]...first.value(r+1]
        for (int i = 1; i < windowSize && i < n; i++) {
            int j = 0;
            matrixD[i][0] = matrixD[i - 1][0] + (Math.pow((first.value(i) - second.value(j)),2)*SlopeWeight+
                    Math.pow((first.value(Math.max(0,i-step)) - second.value(Math.max(0,j-step))),2)*(3-2*SlopeWeight)+
                    Math.pow((first.value(Math.min(n-1,i+step)) - second.value(Math.min(m-1,j+step))),2)*SlopeWeight)/3;
        }
    //Warp the rest,
        for (int i = 1; i < n; i++) {
            ...
            for (int j = start; j < end; j++) {
                ...
                matrixD[i][j] = minDist + (Math.pow((first.value(i) - second.value(j)),2)*SlopeWeight+
                        Math.pow((first.value(Math.max(0,i-step)) - second.value(Math.max(0,j-step))),2)*(3-2*SlopeWeight)+
                        Math.pow((first.value(Math.min(n-1,i+step)) - second.value(Math.min(m-1,j+step))),2)*SlopeWeight)/3;
...
//        String datasetName = "SonyAiboRobotSurface1";
        ...
        // new version
        CDTWSlopeCV1NN dtwNew = new CDTWSlopeCV1NN(r);
    ...
    public static void UCR2018(){
        boolean cv = true;
        Efficient1NN dtwNew = new CDTWSlopeCV1NN();
        dtwNew.cparamId = 100;
...
//                this.step = 0.1 * ((double)(paramId-1)/this.cparamId);
            this.step = 0.1 * ((double)(paramId)%10);
            this.SlopeWeight = 1.5 * ((double)(paramId-paramId%10)/10);

            //this.step = this.steps[paramId];
    ...
    public String getParamInformationString() {
        return this.r+","+this.step+","+this.SlopeWeight;
    }
