 ...
 * have predict accuracy record
 * s and L construct a one-dimentional convolution
 * DTW bidirection
 */
public class CDTWCV1NNConvBi extends Efficient1NN {
    private double r;
    private double step;
    private int L;
     ...
     * @param r
     */
    public CDTWCV1NNConvBi(double r){
...
//        Instance firstReverse = new DenseInstance(first);
        Instance firstReverse = (Instance) first.copy();
        Instance secondReverse = (Instance) second.copy();
//        Instance secondReverse = new DenseInstance(second);
...
//        System.out.println(firstReverse);
        for(int i =0;i<n;i++){
            firstReverse.setValue(i,first.value(n-1-i));
//            firstReverse.setValue(i,n-1-i);
        }
        for(int i =0;i<m;i++){
            secondReverse.setValue(i,second.value(m-1-i));
//            secondReverse.setValue(i,m-1-i);
        }
//        System.out.println("first");
//        System.out.println(first);
//        System.out.println("firstReverse");
//        System.out.println(firstReverse);
        double[] res ={distanceUnidireciton(first,second,cutoff),
                distanceUnidireciton(firstReverse,secondReverse,cutoff)};
        System.out.println("distance1 - distance2: "+(res[0] - res[1])/res[0]);
        return (res[0]+res[1])/2;
    }
    public final double distanceUnidireciton(Instance first, Instance second, double cutoff){
...
//        String datasetName = "SonyAiboRobotSurface1";
        ...
        // new version
        CDTWCV1NNConvBi dtwNew = new CDTWCV1NNConvBi(r);
    ...
    public static void UCR2018(){
        ...
        Efficient1NN dtwNew = new CDTWCV1NNConvBi();
        dtwNew.cparamId = 11*5;
        dtwNew.UCR2018Cls(cv);
    }

    /**
     * @Author :tcl
     * validate for step [0,0.01,...,0.99]
     *  L =[1,2,...,10]
     * Author :tcl
     */
    public static void UCR2018StepAnalyze(){
        Efficient1NN dtwNew = new CDTWCV1NNConvBi();
        dtwNew.cparamId = 10*5;
        try {
...
//            UCR2018StepAnalyze();
    ...
    public void setParamsFromParamId(Instances train, int paramId) {
        if(this.allowLoocv){
            ...
            this.step = 0.2 * ((double)paramId%10/10);
            this.L = (int)Math.floor(paramId/10)*2+1;
