...
/**
 * @author sjx07ngu
 * validate g and bandSize
 * step = 0.05
 */
public class CERP1NN extends Efficient1NN{
    ...
    public CERP1NN(double g, double bandSize) {
        this.g = g;
            ...
            for (int j = l; j <= r; j++) {
                if (Math.abs(i - j) <= band) {
                    ...
                    double diff = (Math.pow((val1 - val2),2)+Math.pow(first.value(Math.max(i-step,0))-
                                  val2,2)+Math.pow(first.value(Math.min(i+step,m-1))-val2,2))/3;
                    ...
                    diff = (Math.pow((val1 - val2),2)+Math.pow(val1-
                            ...
                            second.value(Math.min(j+step,n-1)),2))/3;
...
//        oldDtw.setR(r);
        ...
        // new version
        CERP1NN dtwNew = new CERP1NN(0.1, 0.1);
    ...
    /**
     * Author :tcl
     * validation for g and bandSize
     */
    public static void UCR2018(){
        double r = 1;
        boolean cv = true;
        Efficient1NN dtwNew = new CERP1NN();
        //dtwNew.turnOnCV();
        //this.cparamId = 10;
