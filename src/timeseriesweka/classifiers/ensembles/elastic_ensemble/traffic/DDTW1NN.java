 ...
 */
public class DDTW1NN extends Efficient1NNTraffic {
     ...
     * @param r
     */
    public DDTW1NN(double r){
        this.allowLoocv = false;
        this.r = r;
        this.derf = new DerivativeFilter(){
            @Override
            public Instances process(Instances data) throws Exception {

                for(int i =0;i<data.numInstances();i++){
                    Instance temp = data.instance(i);
//                    double pre = temp.value(1);
                    double len = temp.value(trafficFeatLen+2-1);
                    for(int j =1;j<len;j++){
                        temp.setValue(j,temp.value(j+1)-temp.value(j));
                    }
                    temp.setValue(0,temp.value(1));
                    temp.setValue((int)len-1,temp.value((int)len-2));
                }
                return data;
            }
        };
        if(r!=1){
            this.classifierIdentifier = "DDTW_Rn_1NN";
        }else{
            this.classifierIdentifier = "DDTW_R1_1NN";
        }
    }
...
//    }
    ...
    public final double distance(Instance first, Instance second, double cutoff){
         ...
         * */
        //next: 注释调窗口
        int windowSize = getWindowSize(Math.max(m,n));
...
//for varying window sizes        
        ...
        matrixD[0][0] = Lp(first.value(0) - second.value(0));
...
//Warp first.value(0] onto all second.value(1]...second.value(r+1]
        for (int j = 1; j < windowSize && j < m; j++) {
            matrixD[0][j] = matrixD[0][j - 1] + Lp(first.value(0) - second.value(j));
        }

//	Warp second.value(0] onto all first.value(1]...first.value(r+1]
        for (int i = 1; i < windowSize && i < n; i++) {
            matrixD[i][0] = matrixD[i - 1][0] + Lp(first.value(i) - second.value(0));
        }
//Warp the rest,
        for (int i = 1; i < n; i++) {
            tooBig = true;
            ...
            for (int j = start; j < end; j++) {
                ...
                matrixD[i][j] = minDist + Lp(first.value(i) - second.value(j));
...
//Find the minimum distance at the end points, within the warping window.
        if( matrixD[n-1][m-1]<1e-10){
            System.out.println();
        }
        return matrixD[n-1][m-1];
    }

    /**the only difference with public final double distance(Instance first, Instance second, double cutoff)
     * return matrixD
     * @Author:tcl
     * @param first
     * @param second
    ...
    public final Pair<Double,double[][]> distanceSavePath(Instance first, Instance second, double cutoff){
        ...
        if(first.classIndex() != first.numAttributes()-1 || second.classIndex()!=second.numAttributes()-1){
            DTW temp = new DTW();
            temp.setR(r);
            //return temp.distance(first, second,cutoff); // diable by tcl
        }
        ...
        int n = first.numAttributes()-1;
        int m = second.numAttributes()-1;
        /*  Parameter 0<=r<=1. 0 == no warp, 1train = df.process(train); == full warp
         generalised for variable window size
         * */
...
//        String datasetName = "SonyAiboRobotSurface1";
    ...
    public static void UCR2018(){
        double r = 1;
        boolean cv = false;
        Efficient1NNTraffic dtwNew = new DDTW1NN(r);
        //dtwNew.turnOnCV();
        //this.cparamId = 10;
