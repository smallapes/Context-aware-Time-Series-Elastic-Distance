...
import timeseriesweka.classifiers.ensembles.elastic_ensemble.TSCConfigures;
...
public abstract class Efficient1NNTraffic extends AbstractClassifier implements SaveParameterInfo{
    ...
    public double classifyInstance(Instance instance) throws Exception {
        ...
        for(Instance i:this.train){
            ...
            if(isTraffic){
//                System.out.println("[Train]"+instance.classValue()+",");
                if(instance.value(trafficFeatLen)!=i.value(trafficFeatLen)){
                    continue;
                }else if(i.value(trafficFeatLen+2)==0){
                    continue;
                }else{
                    System.out.print("[TrainThis] ");
                    System.out.print(instance.value(trafficFeatLen)+", "+i.value(trafficFeatLen)+", "+
                                    instance.value(trafficFeatLen+3-1)+", "+i.value(trafficFeatLen+3-1)+", ");
                }
            }
            thisDist = distance(instance, i, bsfDistance);
            if(isTraffic)
                System.out.println(instance.value(trafficFeatLen+4-1)+" ,"+i.value(trafficFeatLen+4-1)+", "+thisDist);
            if(thisDist < bsfDistance){
                ...
                if(!isTraffic){
                    ...
                    // next: other classValue()
                    preClass = i.value(trafficFeatLen+4-1);
                }
            }else if(thisDist==bsfDistance){
                if(!isTraffic) classCounts[(int)i.classValue()]++;
            }
//            System.out.println(369+","+instance.classValue()); //369,1.0105151003E13
//            System.out.println(3691+","+instance.classAttribute()); //3691,@attribute target numeric
//            System.out.println(600+","+i.value(600)); //600,1.0063171000243E13
        }
     ...
     * @throws Exception
     */
    public double classifyInstanceMPuse(Instance instance) throws Exception {
            ...
            class classifyOne implements Runnable{
                ...
                public void run() {
                    ...
                    while(true){
                        ...
                        //System.out.println(600+"attribute,"+i.attribute(600));
                        synchronized (lock2) {
                            if (thisDist < bsfDistance) {
                                bsfDistance = thisDist;
                                classCounts = new int[train.numClasses()];
                                ...
                                //不使用i.classValue(),用attribute取获取值
                                classCounts[(int) i.classValue()]++;
                            }
                        }
                    }
...
//            System.out.print(paramId+" ");
            ...
            // jump to validateId
            if(saveCheckPoint&&paramId<checkPoint.getValidateId()){
...
//            System.out.println(paramId+","+this.getParamInformationString()+","+accAndPreds[0]);
            ...
            if(accAndPreds[0]>bsfAcc){
                bsfAcc = accAndPreds[0];
                bsfParamId = paramId;
                if(saveCheckPoint) checkPoint.setValidateBest(new Pair<>(bsfParamId,bsfAcc));
                bsfaccAndPreds = accAndPreds;
            }

            // save for every validate
            if(saveCheckPoint) checkPoint.setValidateId(paramId);
            if(saveCheckPoint) checkPoint.saveParas();
            if(!this.allowLoocv){
                paramId = paramIdTmp;
            }
        }
//        System.out.println(this.classifierIdentifier+", bsfParamId "+bsfParamId);
        ...
        bw.close();
        if(this.allowLoocv){
            if(saveCheckPoint) bsfParamId = checkPoint.getValidateBest().getKey();
            this.setParamsFromParamId(train, bsfParamId);
        }    
        if(fileWriting){
            FileWriter out = new FileWriter(parsedFileName);
...
//        System.out.println(this.classifierIdentifier+", bsfParamId "+bsfParamId);
    ...
    public double[] loocvAccAndPreds(Instances train, int paramId) throws Exception{
        ...
        for(int i = 0; i < train.numInstances(); i++){
            ...
            if(saveCheckPoint&&i<checkPoint.getValidateNum()){
            ...
            // last:loocv next:WDTW1NNTraffic
            if(isTraffic){
                if(testLoocv.value(trafficFeatLen+3-1)!=0.0){
                    continue;
                }
                System.out.println("[Traffic][Validate] case: " + i+","+testLoocv.value(trafficFeatLen+3-1));
                actual = testLoocv.value(trafficFeatLen+4-1);
                countPos++;
            }else{
                actual = testLoocv.classValue();
            }
            this.buildClassifier(trainLoocv);
            // pred是整数，可以用交通项目的statid_vehid/posid_num_gpsid作为数据的标记
            pred = this.classifyInstance(testLoocv);
            //交通项目: pred==actual的最后5相等就可以了
            if((!isTraffic)&&pred==actual){
                correct++;
            }
        ...
        if(isTraffic){
            accAndPreds[0] = (double)correct/countPos;
            System.out.println("[Traffic][Validate][Accuracy] "+paramId+","+accAndPreds[0] +", "+correct+", "+countPos);
        }else{
            System.out.println(accAndPreds[0]);
    ...
    public double[] loocvAccAndPredsMP(Instances train, int paramId) throws Exception{
        ...
        // else we already know what the params are, so don't need to set
        Efficient1NNTraffic outoutTemp = this;
        ...
        class Out {
            ...
            class MultiProcess implements Runnable{
                Efficient1NNTraffic outout = outoutTemp;
                ...
                public void run() {
                    while(true) {
                        ...
                        int i = i_lock;
                        if (saveCheckPoint&&i_lock < checkPoint.getValidateNum()) {
                            i = checkPoint.getValidateNum();
                            correct = checkPoint.getValidateCorrect();
                            System.out.println("[checkpoint] validate id: " + checkPoint.getValidateId() +
                                    " validateNum: " + checkPoint.getValidateNum() + "validateCorrect: " + checkPoint.getValidateCorrect());
...
//
            ...
            while((line=br.readLine())!=null){
                ...
                if(saveCheckPoint&&!datasetName.equals(checkPoint.getDatasetName())){
                ...
                long start, end, newTime=0;
                System.out.println(1564+":"+trainTestFold);
                for(int foldId=0;foldId<trainTestFold;foldId++) {
                    ...
                    if(saveCheckPoint&&this.allowLoocv)
                        setParamsFromParamId(train,checkPoint.getValidateBest().getKey());

                    FileWriter trafficout =null;
                    if(isTraffic){
                        trafficout = new FileWriter(new File(TSCConfigures.trafficResultFile+totalClassName[totalClassName.length-1]+".txt"));
                    }
                    for (int i = 0; i < test.numInstances(); i++) {
                        if(isTraffic&&test.instance(i).value(trafficFeatLen+2)==0){
                            System.out.print("[TEST] "+i+" ");
                            System.out.println(test.instance(i).value(trafficFeatLen));
                        }else{
                            System.out.print("[Traffic] NOT TEST Case,");
                            System.out.print(test.instance(i).value(trafficFeatLen+2)+", ");
                            System.out.println(test.instance(i).value(trafficFeatLen+4-1));
                            continue;
                        }

                        if(i%10==0&&!isTraffic)
                            System.out.println("[Loop] Test case: " + i);
                        if(saveCheckPoint&&i<checkPoint.getTestNum()&&!isTraffic){
                        ...
                        // next: classifyInstance()
                        if (!isTraffic&&pred == test.instance(i).classValue()) {
                            correctNew++;
                            if(saveCheckPoint&&i%10==0){
                        ...
                        System.out.println(test.instance(i).value(trafficFeatLen+4-1)+","+pred);
                        System.out.println();
                        if(isTraffic&&pred ==  test.instance(i).value(trafficFeatLen+4-1)){
                            correctNew++;
                        }
                        if(isTraffic&&trafficout!=null){
                            Instance temp = test.instance(i);
                            trafficout.write(temp.value(trafficFeatLen)+","
                                            +temp.value(trafficFeatLen+5-1)+","
                                            +pred+"\n");
                            trafficout.flush();
                        }

                        System.out.println("correctNew: "+correctNew+","+(i+1));
                    ...
                    if(trainTestFold==1){
                        ...
                        if(datasetSavePath.equals("None")) {
                            if (cv) {
                                bw.write(datasetName + "," + ((double) correctNew / test.numInstances()) + "," + newTime + "," + this.getParamInformationString() + "\n");
                            } else {
                        ...
                        System.out.println("New acc:    "+((double)correctNew/test.numInstances()));
                        System.out.println("New timing: "+(end-start));
                    }
                }
                ...
                if(isTraffic) break;// only onedataset
            }
            bw.close();
        }catch (Exception e){
            e.printStackTrace();
