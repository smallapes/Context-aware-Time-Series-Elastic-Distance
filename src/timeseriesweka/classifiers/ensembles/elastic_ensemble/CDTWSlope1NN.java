 ...
 *
 * @author tcl
 * set slope weight to context point of current point
 * SlopeWeight  = 0.5
 * abandoned
 */
public class CDTWSlope1NN extends Efficient1NN {
    ...
    public CDTWSlope1NN(double r, double step){
        this.allowLoocv = false;
        this.r = r;
        if(r!=1){
            this.classifierIdentifier = "DTW_Rn_1NN";
...
//for varying window sizes        
        ...
        for(int i=0,j=0;i<1;i++){
            matrixD[0][0] = (Math.pow((first.value(i) - second.value(j)),2)*0.5+
                    Math.pow((first.value(Math.max(0,i-step)) - second.value(Math.max(0,j-step))),2)*2+
                    Math.pow((first.value(Math.min(n-1,i+step)) - second.value(Math.min(m-1,j+step))),2)*0.5)/3;
        }
...
//Warp first.value(0] onto all second.value(1]...second.value(r+1]
        for (int j = 1; j < windowSize && j < m; j++) {
            int i = 0;
            matrixD[0][j] = matrixD[0][j - 1] + (Math.pow((first.value(i) - second.value(j)),2)*0.5+
                    Math.pow((first.value(Math.max(0,i-step)) - second.value(Math.max(0,j-step))),2)*2+
                    Math.pow((first.value(Math.min(n-1,i+step)) - second.value(Math.min(m-1,j+step))),2)*0.5)/3;
        }
...
//	Warp second.value(0] onto all first.value(1]...first.value(r+1]
        for (int i = 1; i < windowSize && i < n; i++) {
            int j = 0;
            matrixD[i][0] = matrixD[i - 1][0] + (Math.pow((first.value(i) - second.value(j)),2)*0.5+
                    Math.pow((first.value(Math.max(0,i-step)) - second.value(Math.max(0,j-step))),2)*2+
                    Math.pow((first.value(Math.min(n-1,i+step)) - second.value(Math.min(m-1,j+step))),2)*0.5)/3;
        }
//Warp the rest,
        for (int i = 1; i < n; i++) {
            ...
            for (int j = start; j < end; j++) {
                ...
                matrixD[i][j] = minDist + (Math.pow((first.value(i) - second.value(j)),2)*0.5+
                        Math.pow((first.value(Math.max(0,i-step)) - second.value(Math.max(0,j-step))),2)*2+
                        Math.pow((first.value(Math.min(n-1,i+step)) - second.value(Math.min(m-1,j+step))),2)*0.5)/3;
...
//        String datasetName = "SonyAiboRobotSurface1";
        ...
        // new version
        CDTWSlope1NN dtwNew = new CDTWSlope1NN(r);
    ...
    public static void UCR2018(){
        double r = 1;
        boolean cv = false;
        Efficient1NN dtwNew = new CDTWSlope1NN(r);
        //dtwNew.turnOnCV();
        //this.cparamId = 10;
    ...
    public static void UCR2018SavePath(){
        double r = 1;
        Efficient1NN dtwNew = new CDTWSlope1NN(r,0.05);
        boolean cv =false;
        boolean resample100 =false;
        dtwNew.UCR2018Cls(cv, resample100,"theory",false);
    }
