...
public class CDTW_DistanceBasic extends EuclideanDistance{
    ...
    public CDTW_DistanceBasic(){
            super();
            m_DontNormalize=true;

    }
    public CDTW_DistanceBasic(Instances data) {
        super(data);
        m_DontNormalize=true;
    }
...
//Find the min of matrixD[i][j-1],matrixD[i-1][j] and matrixD[i-1][j-1]
                if (i < j + windowSize && j < i + windowSize) {
                    ...
                    matrixD[i][j]=minDist+((a[i]-b[j])*(a[i]-b[j])+
                                            Math.pow(a[Math.max(i-step,0)]-b[Math.max(j-step,0)],2)+
                                            Math.pow(a[Math.min(i+step,n-1)]-b[Math.min(j+step,m-1)],2))/3;
